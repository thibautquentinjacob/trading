...
export class AppComponent {
    ...
    private _currentStockSymbol: string;

    constructor ( private _portfolioService: PortfolioService ) {

        // Default is AAPL
        this._currentStockSymbol = 'FB';
        ...
        this.subject.subscribe(
            ( msg: any ) => {
                ...
                msg._quotes.forEach( element => {
                    ...
                    if ( buyDecision.decision && ( buyDecision.amount > 0 || buyDecision.amount === -1 )) {
                        ...
                        if ( buyDecision.amount === -1 && amount > 0 ) {
                            this._portfolioService.buy( new Stock( this._currentStockSymbol, amount ), element.open );
                            markers.push({
                                name: `Buying ${amount} x ${this._currentStockSymbol} for ${element.open * amount} at ${transformedDate}`,
                                type: 'buy',
                                yAxis: element.open,
                                xAxis: transformedDate
                            });
                        } else if ( buyDecision.amount !== -1 ) {
                            this._portfolioService.buy( new Stock( this._currentStockSymbol, buyDecision.amount ), element.open );
                            markers.push({
                                name: `Buying ${buyDecision.amount} x ${this._currentStockSymbol} for ${element.open * buyDecision.amount} at ${transformedDate}`,
                                type: 'buy',
                                yAxis: element.open,
                                xAxis: transformedDate
                            });
                    ...
                    if ( sellDecision.decision && ( sellDecision.amount > 0 || sellDecision.amount === -1 )) {
                        const amount: number = this._stocks[this._currentStockSymbol] ? this._stocks[this._currentStockSymbol].amount : 0;
                        if ( sellDecision.amount === -1 && amount > 0 ) {
                            this._portfolioService.sell( new Stock( this._currentStockSymbol, amount ), element.open );
                            markers.push({
                            ...
                            this._portfolioService.sell( new Stock( this._currentStockSymbol, sellDecision.amount ), element.open );
                            markers.push({
                                name: `Selling ${sellDecision.amount} x ${this._currentStockSymbol} for ${element.open * sellDecision.amount} at ${transformedDate}`,
                                type: 'sell',
                                yAxis: element.open,
                                xAxis: transformedDate
                            });
                ...
                this.chartOption = {
                    ...
                    series: [
                        {
                            name: `${this._currentStockSymbol} day evolution`,
                            type: 'candlestick',
                            xAxisIndex: 0,
                            yAxisIndex: 0,
                            data: ohlc,
        ...
        this.subject.next({
            command: 'GET_QUOTE',
            options: {
                quote: this._currentStockSymbol
            }
        });
    }
}
